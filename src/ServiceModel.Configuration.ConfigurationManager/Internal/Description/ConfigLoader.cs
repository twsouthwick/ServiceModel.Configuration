...
namespace System.ServiceModel.Description
{
    ...
    class ConfigLoader
    {
        ...
        internal void LoadChannelBehaviors(ServiceEndpoint serviceEndpoint, string configurationName)
        {
            ...
            {
                serviceEndpoint.Address = new EndpointAddress(channelElement.Address, LoadIdentity(channelElement.Identity), channelElement.Headers.Headers.ToArray());
            }
            ...
            {
                LoadBehaviors<IEndpointBehavior>(commonBehaviors.EndpointBehaviors, GetServiceBehaviors(serviceEndpoint), true/*commonBehaviors*/);
            }
            ...
            {
                LoadBehaviors<IEndpointBehavior>(behaviorElement, GetServiceBehaviors(serviceEndpoint), false/*commonBehaviors*/);
            }
        }

        private static KeyedByTypeCollection<IEndpointBehavior> GetServiceBehaviors(ServiceEndpoint s)
        {
#if DESKTOP
            return s.Behaviors;
#else
        ...
        {
            return new EndpointAddress(element.Address, LoadIdentity(element.Identity), element.Headers.Headers.ToArray());
        }
            ...
            EndpointAddress address, ContextInformation context, ContractDescription contract, out ServiceEndpoint endpoint)
        {
            ...
            {
                endpoint.Address = new EndpointAddress(channelEndpointElementCopy.Address, LoadIdentity(channelEndpointElementCopy.Identity), channelEndpointElementCopy.Headers.Headers.ToArray());
            }
            ...
            if (commonBehaviors != null && commonBehaviors.EndpointBehaviors != null)
            {
                LoadBehaviors<IEndpointBehavior>(commonBehaviors.EndpointBehaviors, GetServiceBehaviors(endpoint), true/*commonBehaviors*/);
            }
            ...
            if (behaviorElement != null)
            {
                LoadBehaviors<IEndpointBehavior>(behaviorElement, GetServiceBehaviors(endpoint), false/*commonBehaviors*/);
            }
