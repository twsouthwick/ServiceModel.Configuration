...
namespace System.ServiceModel.Description
{
    ...
    class ConfigLoader
    {
        ...
        static BindingCollectionElement GetBindingCollectionElement(string bindingSectionName, ContextInformation context)
        {
            ...
            {
                throw new ConfigurationErrorsException("ERROR!");// SR.GetString(SR.ConfigBindingTypeCannotBeNullOrEmpty));
            }
            if (context == null)
            {
                // If no context is passed in, assume that the caller can consume the AppDomain's
                    ...
                    if (bindingElement != null)
                    {
                        if (bindingElement.Name.Equals(configurationName, StringComparison.Ordinal))
                        {
                            ...
                            if (ConfigLoader.resolvedBindings.Contains(resolvedBindingID))
                            {
                                ...
                                throw new ConfigurationErrorsException("ERROR!");// SR.GetString(SR.ConfigBindingReferenceCycleDetected, detectedCycle.ToString()), configErrorElement.ElementInformation.Source, configErrorElement.ElementInformation.LineNumber));
                            }
